...
from inversion_methods.amxkf.data_amxkf import DataConfig, extract_data
from inversion_methods.amxkf.inversion_amxkf import amxkf_monthly_dictionaries, augmented_mxkf, amxkf_postprocessouts, MessyInput, PostProcessInput
...
def amxkf_function(config: InversionParameters):
    ...
    inversion_input = amxkf_monthly_dictionaries(messy_input)

    end_data = time.time()

    print(f"Data extraction and preparation complete. Time taken = {end_data-start_data:.4f} seconds")

    start_amxkf = time.time()

    za, za_mu, Pa = augmented_mxkf(inversion_input)

    
    end_amxkf = time.time()

    print(f"AMXKF Complete. Time taken = {end_amxkf-start_amxkf:.4f} seconds")

    start_post = time.time()

    post_process_input = PostProcessInput(z=za,
                                          z_mu=za_mu,
                                          Pz=Pa,
                                          sigma_rep=config.sigma_rep,
                                          sigma_qx=config.sigma_qx,
                                          kappa=config.kappa_x,
    ...
    outsds = amxkf_postprocessouts(post_process_input)
