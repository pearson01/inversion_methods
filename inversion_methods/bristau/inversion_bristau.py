...
#     # xoutprior_sigma2s = np.ones(nxout) * xprior_sigma**2
#     # xinprior_sigma2s = np.ones(nxin) * xprior_sigma**2
#     # xprior_sigma2s = np.concatenate((xoutprior_sigma2s, xinprior_sigma2s))

#     # xoutprior_mu = np.log(1.3) * np.ones(nxout)
#     # xinprior_mu = np.log(1.3) * np.ones(nxin)
#     # xprior_mus = np.concatenate((xoutprior_mu, xinprior_mu))

#     # routprior_mu = np.log(0.9)
#     # rinprior_mu = np.log(1.3)
#     # rprior_mus = np.array([routprior_mu, rinprior_mu])
...
#     sigma2_qxout_current = 0.02
#     sigma2_qxin_current = np.full(nxin, 0.02)
...
#     # kappa_x_current = 0
#     kappa_x_max = kappa_max(config.nperiod, 3)
...
#     var_qxin_trace = np.zeros((config.iterations, config.ningroup))
...
#         print(f"Iteration:{i}, sigma2_rep: {sigma2_rep_current}, sigma2_qx min/max: ({min(sigma2_qxin_current)}, {max(sigma2_qxin_current)}), kappa: {kappa_xin_current}", flush=True)
#         # print(f"Iteration:{i}, sigma2_rep: {sigma2_rep_current}, sigma2_qxin: {sigma2_qxin_current}, kappa: {kappa_xin_current}", flush=True)

#         # if i > 0 and i % 100 == 0:
#         #     print(f"Gibbs iteration {i}", flush=True)
        
#         xoutprior_sigma2 = (sigma2_qxout_current + sigma2_qr) / (1 - kappa_xout_current**2)
#         xoutprior_sigma2 = min(xoutprior_sigma2, xprior_sigma**2)
#         # xoutprior_sigma2 = min(xoutprior_sigma2, sigma2_qx_max)
#         xoutprior_sigma2s = np.ones(nxout) * xoutprior_sigma2

#         # xinprior_sigma2 = (sigma2_qxin_current + sigma2_qr) / (1 - kappa_xin_current**2)
#         # xinprior_sigma2 = min(xinprior_sigma2, xprior_sigma**2)
#         # xinprior_sigma2s = np.ones(nxin) * xinprior_sigma2

#         # # elementwise now, since sigma2_qxin_current is a vector
#         xinprior_sigma2s = (sigma2_qxin_current + sigma2_qr) / (1 - kappa_xin_current**2)
#         xinprior_sigma2s = np.minimum(xinprior_sigma2s, xprior_sigma**2)
#         # xinprior_sigma2s = np.minimum(xinprior_sigma2s, sigma2_qx_max)
...
#         sigma2_qxs = np.concatenate((sigma2_qxouts, sigma2_qxin_current))
...
#                                      kappa_xout=kappa_xout_current,
#                                      kappa_xin=kappa_xin_current,
#                                      F_aug=F_aug,
...
#         print(f"            z sample median: {np.median(zsample)}, z sample mean: {np.mean(zsample)}, z sample std: {np.std(zsample)}")
...
#         sigma2_qxin_current, sigma2_qxin_group_current = sample_sigma2_qx_grouped(zmusample_in, kappa_xin_current, sigma2_qx_aprior, sigma2_qx_bprior, config.inner_group_id, config.ningroup, sigma2_qx_max)
#         # sigma2_qxout_current = 0.09
#         # sigma2_qxin_current = 0.25
...
#         var_qxin_trace[i] = sigma2_qxin_group_current
...
#     else:
#         return xtrace[burn:], None, rtrace[burn:], var_rep_trace[burn:], var_qxout_trace[burn:], var_qxin_trace[burn:], kappaouttrace[burn:], kappaintrace[burn:]
    ...
    for period in np.arange(config.nperiod):
        ...
        for ci, cntry in enumerate(cntrynames):
            ...
            for bf in range(int(np.max(bfarray)) + 1):
                ...
                # cntrymedian[ci, period] += weight_bf * xouts_median[period, bf]

                cntrytottrace += weight_bf * config.xtrace[:, period, bf]
                cntrytotprior += weight_bf
            
            # cntrytracemedian = np.median(cntrytottrace)
            # cntrymedianbias = cntrytracemedian - cntrymedian[ci, period]

            # cntrytrace68 = az.hdi(cntrytottrace, 0.68)
            # cntrytrace95 = az.hdi(cntrytottrace, 0.95)
            # cntry68[ci, :, period] = cntrytrace68 - cntrymedianbias
            # cntry95[ci, :, period] = cntrytrace95 - cntrymedianbias
